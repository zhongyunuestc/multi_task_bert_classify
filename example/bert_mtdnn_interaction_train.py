...
def read_data_from_tfrecord_v2(input_file, max_seq_length, batch_size, is_training, epochs, drop_remainder=False):
    name_to_features = {
        ...
        "is_real_example": tf.FixedLenFeature([], tf.int64),
        "label_ids": tf.FixedLenFeature([], tf.int64),
        "addon_input_ids": tf.FixedLenFeature([max_seq_length], tf.int64),
        "addon_input_mask": tf.FixedLenFeature([max_seq_length], tf.int64),
        "addon_segment_ids": tf.FixedLenFeature([max_seq_length], tf.int64),
...
def create_model_v2(bert_config,
    ...
    addon_input_ids = tf.placeholder(tf.int32, [None, None], name='addon_input_ids')
    addon_input_mask = tf.placeholder(tf.int32, [None, None], name='addon_input_mask')
    addon_segment_ids = tf.placeholder(tf.int32, [None, None], name='addon_segment_ids')
    ...
    addon_model = modeling.BertModel(config=bert_config,
                               is_training=is_training,
                               input_ids=addon_input_ids,
                               input_mask=addon_input_mask,
                               token_type_ids=addon_segment_ids,
                               use_one_hot_embeddings=use_one_hot_embeddings,
                               scope='bert')
    ...
    loss, pairsent_loss, final_loss, logits, probabilities, acc, pairsent_acc, pred_labels = inference(model, addon_model, num_labels, 
    ...
    return (input_ids, input_mask, segment_ids, label_ids, is_training,
            addon_input_ids, addon_input_mask, addon_segment_ids,
...
def inference(model, addon_model, num_labels, is_training, labels, pairsent_labels):
    ...
    #pairsent_output_layer = addon_model.get_pooled_output()
    pairsent_output_layer = tf.reduce_mean(addon_model.get_sequence_output(), 1)
    ...
    pairsent_w = tf.get_variable(
                        "pairsent_output_weights", [2, hidden_size],
                        initializer=tf.truncated_normal_initializer(stddev=0.02))
    ...
    infer_pairsent_output_layer = tf.placeholder(tf.float32, [None, None], name='infer_pairsent_outputs')
    infer_pairsent_logits = tf.matmul(infer_pairsent_output_layer, pairsent_w, transpose_b=True) 
    infer_pairsent_logits = tf.nn.bias_add(infer_pairsent_logits, pairsent_bias, name='infer_pairsent_logits')
    ...
    with tf.variable_scope("loss"):
        ...
        one_hot_pairsent_labels = tf.one_hot(pairsent_labels, depth=2, dtype=tf.float32)
        pairsent_losses = tf.nn.softmax_cross_entropy_with_logits(logits=pairsent_logits, labels=one_hot_pairsent_labels)
        #pairsent_losses = tf.losses.mean_squared_error(labels=one_hot_pairsent_labels, predictions=tf.nn.sigmoid(pairsent_logtis))
        #pairsent_losses = tf.contrib.losses.metric_learning.triplet_semihard_loss(labels=pairsent_labels, embeddings=pairsent_logits, margin=.5)
    ...
    with tf.name_scope("accuracy"):
        ...
        pairsent_correct_pred = tf.equal(tf.argmax(one_hot_pairsent_labels, 1), pairsent_pred_labels)
...
def file_based_convert_examples_to_features_v2(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        addon_feature = convert_single_example(ex_index, addon_example, addon_label_map, max_seq_length, tokenizer)

        features["addon_input_ids"] = create_int_feature(addon_feature.input_ids)
        features["addon_input_mask"] = create_int_feature(addon_feature.input_mask)
        features["addon_segment_ids"] = create_int_feature(addon_feature.segment_ids)
...
def eval_process(sess,
    ...
    for i in range(num_eval_steps):
        ...
        feed_dict = {input_ids: batch_data['input_ids'],
                     ...
                     addon_input_ids: batch_data['addon_input_ids'],
                     addon_input_mask: batch_data['addon_input_mask'],
                     addon_segment_ids: batch_data['addon_segment_ids'],
