...
tf.flags.DEFINE_string("question_person_map_file", DATA_DIR + '/question_to_user_id.txt', "candidates data source.")
...
def show_best_person(preds, y_truth, idx2label, cand2person):
    pred_persons = [cand2person[idx2label[int(item)]] for item in preds]
    y_truth_persons = [cand2person[idx2label[int(item)]] for item in y_truth]
    
    correct_predictions = 0.0
    for left, right in zip(pred_persons, y_truth_persons):
        if left == right:
...
def write_predictions(raw_examples, features, all_predictions, all_topk, idx2label, cand2person):
    #get real label
    ...
    show_best_person(all_predictions, y_truth, idx2label, cand2person)
...
def read_question_person_map(file_name):
    cand2person = {}
    with codecs.open(file_name, 'r', 'utf8') as fr:
        for line in fr:
            ...
            cand = line_info[0]
            person_name = line_info[1]
            person_id = line_info[2]
            cand2person[cand] = (person_name, person_id)
    return cand2person

def eval():
    cand2person = read_question_person_map(FLAGS.question_person_map_file)
    ...
    write_predictions(raw_examples, features, all_predictions, all_topk, idx2label, cand2person)
