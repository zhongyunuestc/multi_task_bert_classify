...
def inference(model, model_a, model_b, num_labels, is_training, labels, pairsent_labels):
    ...
    infer_a_outputs = tf.placeholder(tf.float32, [None, None], name='infer_a_outputs')
    infer_b_outputs = tf.placeholder(tf.float32, [None, None], name='infer_b_outputs')
    infer_pairsent_output_layer = tf.concat([infer_a_outputs, infer_b_outputs], -1)
    ...
    with tf.variable_scope("loss"):
        ...
        #pairsent_losses = tf.losses.mean_squared_error(labels=one_hot_pairsent_labels, predictions=tf.nn.sigmoid(pairsent_logits))
        pairsent_losses = tf.contrib.losses.metric_learning.triplet_semihard_loss(labels=pairsent_labels, embeddings=pairsent_logits, margin=0.5)
        #pairsent_losses = tf.contrib.losses.metric_learning.triplet_semihard_loss(labels=pairsent_labels, embeddings=pairsent_output_layer, margin=0.5)
