...
def eval():
    ...
    names = read_labels(FLAGS.label_file)
    name2idx, idx2name = build_dict(names)
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            for sentence_feature, feature in zip(all_sentence_features, features):
                ...
                #cand_ids = [cand_input_features[int(i) - 1].input_ids for i in absolute_ids]
                ...
                best_sent_id = np.argmax(batch_probs_value)
                truth_user_id = feature.label_id
        
                predicts.append(candidates[best_sent_id]) 
                if cand2user[candidates[best_sent_id]][0] == idx2name[truth_user_id]:
                    user_corrects += 1
                count += 1
                
    print('count:', count)
    print('corrects:', user_corrects)
